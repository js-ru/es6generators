# Глубокое погружение в генераторы ES6 {#es6_generators_dive}
...
Если вы ещё не знакомы с генераторами ES6, сначала прочитайте и поиграйтесь с кодом в статье «[Часть 1: Основы генераторов ES6](#es6_generators_basic)». Как только вы решите, что поняли основы, мы погрузимся в некоторые более глубокие детали.

## Обработка ошибок

Одна из наиболее важных частей при проектировании ES6 генераторов заключается в том, что семантика кода внутри генератора является **синхронной**, даже если внешний контроль осуществляется асинхронно.
...
**Примечание:** если `throw(...)` выбрасывает ошибку в генератор, но `try...catch` не ловит её, ошибка (как и в обычной функции) распространится обратно (и если не поймать её, в конечном итоге окажется необработанным исключением). Вот так:
...
Ещё кое-что, что, вам, возможно, захочется сделать — вызвать другой генератор внутри функции-генератора. Я имею в виду не инициализацию генератора обычным способом, и именно делегирование управления другому генератору. Чтобы это сделать, используется вариант ключевого слова `yield`: `yield *` («yield звёздочка»).
...
Для простоты, этот пример только отдаёт (`yield`) значения наружу. Но конечно, если вы не используете цикл `for...of`, а вручную вызываете метод итератора `next(...)` и передаёте сообщения, эти сообщения передаются через `yield*` таким же ожидаемым способом:
...
Хотя я показал здесь только один уровень делегирования, `*foo()` может с помощью `yield*` делегировать управление другому генератору, затем следующему и так далее.
...
Как видите, `yield *foo()` делегирует управление (вызовы `next()`) генератору `foo()`, пока он не завершится. Когда это произойдёт, любое возвращаемое значение (в данном случае это строка `"foo"`) станет результатом выражения `yield*` и будет присвоено локальной переменной `v`.
...
Основная задача будет заключаться в создании механизма, в котором генератор останавливается для запуска асинхронной задачи, а затем возобновляется (с помощью вызова метода `next()` итератора), когда асинхронная задача завершается. В следующей статье мы рассмотрим различные способы создания такого управления асинхронностью с помощью генераторов. Будьте на связи!
