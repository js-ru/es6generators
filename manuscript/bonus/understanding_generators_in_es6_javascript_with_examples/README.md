...
*Проблема: Мы хотим создать собственный итерируемый объект, который будет возвращать `This `, `is ` и `iterable.`. Вот одна реализация с помощью итераторов —*
...
- Нам не нужно вручную создавать объект, который вернёт метод `next()`, т.е. `{ value: 'This', done: false }`.
