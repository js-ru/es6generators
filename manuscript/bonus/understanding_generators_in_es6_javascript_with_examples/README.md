...
![Обычные функции против генераторов](https://github.com/adam-p/markdown-here/raw/master/src/common/images/icon48.png "Обычные функции против генераторов")
...
*Проблема: Мы хотим создать кастомный итерируемый объект, который будет возвращать `This `, `is `, и `iterable.`. Вот одна реализация с помощью итераторов —*
...
- Мы не должны беспокоиться о `Symbol.iterator`.
- Мы не должны реализовывать метод `next()`.
- Мы не должны вручную создавать объект, который вернёт метод `next()`, т.е. `{ value: 'This', done: false }`.
- Мы не должны сохранять состояние. В примере с итератором состояние сохраняется в переменную `step`. От значения этой переменной зависит значение, которое отдаётся итератором. В случае с генератором нам не нужно делать ничего из этого.
...
Ещё многое можно рассмотреть в теме генераторов. Например, такие вещи, как `yield *`, `return()` и `throw()`. Генераторы также позволяют реализовывать [корутины](https://en.wikipedia.org/wiki/Coroutine). Я перечислил некоторые ссылки, по которым вы можете прочитать дополнительную информацию.
...
- Углубленная серия из четырех частей [Кайла Симпсона](https://medium.com/@getify) о генераторах и корутинах. Можно прочитать [здесь](https://davidwalsh.name/es6-generators).
